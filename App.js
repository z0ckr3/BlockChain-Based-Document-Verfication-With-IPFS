...
window.onload = async () => {
  ...
  if (window.ethereum || window.location.pathname === "/verify.html") {
    ...
    window.contract = new window.web3.eth.Contract(
     ...
     if(window.userAddress.length>10){ listen();}

    if (window.userAddress.length>5 || window.location.pathname === "/verify.html" )  {
...
function print_info(result, is_verified) {
  ...
  if (!is_verified) {
    ...
    document.getElementById("download-document").classList.remove("d-none")
...
async function listen() {
  ...
  window.web3 = new Web3(window.ethereum);
  window.contract = new window.web3.eth.Contract(window.ABI, CONTRACT_ADDRESS);
  await window.contract
    .getPastEvents(
      "addHash",
      {
        filter: {
         _exporter : window.userAddress   //Only get events where the id matches a certain value
      }, 
        fromBlock: (await window.web3.eth.getBlockNumber()) - 999,
        toBlock: "latest",
      },
      function (error, events) { 
        printTransactions(events, events.length);
        console.log(events);
      }
    )
...
function printTransactions(data, length) {
  document.querySelector(".loading-tx").classList.add("d-none")
  if(!length){document.querySelector(".tx-h5").classList.add("d-none"); return;}
